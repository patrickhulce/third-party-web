...
FROM (
  ...
  FROM (
    SELECT
      ...
      SAFE_CAST(JSON_VALUE(bootupTimeItems,"$.scripting") AS FLOAT64) AS executionTime
    FROM (
      SELECT url AS page, report
      FROM `httparchive.lighthouse.2022_01_01_mobile`
    ),
    UNNEST(JSON_QUERY_ARRAY(report,'$.audits.bootup-time.details.items')) AS bootupTimeItems
    INNER JOIN `lighthouse-infrastructure.third_party_web.2022_01_01`
    ON NET.REG_DOMAIN(JSON_VALUE(bootupTimeItems, "$.url")) = domain
  )
